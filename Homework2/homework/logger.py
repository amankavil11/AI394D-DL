...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        logger.add_scalar("train_accuracy", torch.tensor(metrics["train_acc"]).mean(), global_step)
        ...
        logger.add_scalar("val_accuracy", torch.tensor(metrics["val_acc"]).mean(), global_step)
